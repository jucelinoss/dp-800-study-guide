...
1. **Schema-Driven Data Products via JSON Metadata**:
   - As demonstrated in the lab, metadata tables generate dynamic T-SQL queries via `sp_executesql`, allowing the Data Product to expose new JSON properties without breaking contracts or requiring database redeployments.
...
* **Microsoft Fabric SQL Database (SaaS)**:
  * **Automatic Mirroring to OneLake**: All tables written to Fabric SQL are mirrored in real time in **Delta Lake / Parquet** format in the central OneLake, without requiring manual ETL pipelines.
