...
--   2. Precise Search Predicates with `CONTAINS` (Prefixes `"capacete*"`, Proximity `NEAR`, Inflection `FORMSOF`)
...
--   5. Custom Stoplist Management (`sys.fulltext_stopwords`)
-- =================================================================================
...
CREATE TABLE lab.FtsProductCatalog (
    ProductID INT IDENTITY(1,1) PRIMARY KEY, -- Unique key required by FTS
...
-- 2. Create the Full-Text Index on ProductName and Description columns
CREATE FULLTEXT INDEX ON lab.FtsProductCatalog (
    ProductName LANGUAGE 1033, -- 1033 = English
    Description LANGUAGE 1033
)
KEY INDEX PK__FtsProdu__B40CC6ED3DFB3AC5 -- Table primary key
ON LabFtsCatalog
...
-- -- [DP-800 KEY POINT]
-- 1. Prefix Search (Word wildcard `"capac*"` or `"fon*"`)
SELECT ProductID, ProductName, Description
FROM lab.FtsProductCatalog
WHERE CONTAINS(Description, '"fon*" OR "capac*"');
GO

-- 2. Proximity Search with NEAR (Words that appear close together)
SELECT ProductID, ProductName, Description
FROM lab.FtsProductCatalog
WHERE CONTAINS(Description, 'NEAR((fones, bluetooth), 5)');
GO

-- 3. Natural Language Search with FREETEXT
SELECT ProductID, ProductName, Description
FROM lab.FtsProductCatalog
