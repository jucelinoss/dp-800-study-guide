-- =================================================================================
-- DP-800 - PRACTICAL LAB: DATA VAULT RAW VAULT AND HISTORIZATION
-- Database: AdventureWorks2025 (or similar)
-- =================================================================================
-- Demonstrates: source snapshots, idempotent Hubs and Links, Satellite history,
-- SHA2_256 hashdiffs, point-in-time querying, and quality gates.
-- This SQL Server script simulates Raw Vault boundaries. Fabric-specific storage
-- and orchestration can use Lakehouse/Warehouse, OneLake, pipelines, notebooks,
-- Spark, or materialized lake views.
-- THEORY: ../../../certification/12-other-topics/03-data-vault-architecture.md
...
-- =================================================================================
-- PART 1: LAND TWO SOURCE SNAPSHOTS
-- =================================================================================
...
VALUES
    ('2026-08-01T08:00:00', N'crm-sales-2026-08-01', N'CRM+ERP', N'C-100', N'Contoso Brasil', N'ACTIVE', N'O-1001', '2026-07-31', N'PLACED', N'P-501', N'Adventure Helmet', 2, 25.00),
    ('2026-08-01T08:00:00', N'crm-sales-2026-08-01', N'CRM+ERP', N'C-200', N'Northwind Bikes', N'ACTIVE', N'O-1002', '2026-07-31', N'PLACED', N'P-502', N'Touring Tire', 1, 80.00),
    -- A later snapshot changes customer context and order status.
    ('2026-08-10T08:00:00', N'crm-sales-2026-08-10', N'CRM+ERP', N'C-100', N'Contoso Brasil Ltda.', N'ACTIVE', N'O-1001', '2026-07-31', N'SHIPPED', N'P-501', N'Adventure Helmet', 2, 25.00),
    ('2026-08-10T08:00:00', N'crm-sales-2026-08-10', N'CRM+ERP', N'C-200', N'Northwind Bikes', N'SUSPENDED', N'O-1002', '2026-07-31', N'CANCELLED', N'P-502', N'Touring Tire', 1, 80.00);
GO
...
-- =================================================================================
-- PART 4: SATELLITES AND HASHDIFF HISTORY
-- =================================================================================
...
SELECT N'Customer history' AS HistoryName, c.CustomerCode AS BusinessKey, sat.LoadDate, sat.CustomerName, sat.CustomerStatus, CONVERT(VARCHAR(64), sat.HashDiff, 2) AS HashDiff
FROM lab_datavault.SatCustomer AS sat JOIN lab_datavault.HubCustomer AS c ON c.CustomerHashKey = sat.CustomerHashKey
UNION ALL
SELECT N'Order history', o.OrderCode, sat.LoadDate, sat.OrderStatus, CONCAT(N'Qty=', sat.Quantity, N'; Price=', sat.UnitPrice), CONVERT(VARCHAR(64), sat.HashDiff, 2)
...
-- =================================================================================
-- PART 5: POINT-IN-TIME QUERY AND BUSINESS VAULT PROJECTION
-- =================================================================================
...
IF EXISTS (SELECT l.OrderHashKey FROM lab_datavault.LinkOrderCustomer AS l LEFT JOIN lab_datavault.HubOrder AS o ON o.OrderHashKey = l.OrderHashKey LEFT JOIN lab_datavault.HubCustomer AS c ON c.CustomerHashKey = l.CustomerHashKey WHERE o.OrderHashKey IS NULL OR c.CustomerHashKey IS NULL)
    THROW 51031, 'Link referential-integrity check failed.', 1;
IF EXISTS (SELECT sat.CustomerHashKey FROM lab_datavault.SatCustomer AS sat LEFT JOIN lab_datavault.HubCustomer AS h ON h.CustomerHashKey = sat.CustomerHashKey WHERE h.CustomerHashKey IS NULL)
    THROW 51032, 'Satellite parent-hub check failed.', 1;
SELECT N'PASS' AS DataVaultQualityGate, N'Hubs, links, satellites, hashdiff history, and parent references are valid.' AS Result;
GO
