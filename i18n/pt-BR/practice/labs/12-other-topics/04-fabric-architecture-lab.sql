-- =================================================================================
-- DP-800 - LABORATÓRIO PRÁTICO: ARQUITETURA DATA FABRIC E MICROSOFT FABRIC
-- Banco de dados: AdventureWorks2025 (ou similar)
-- =================================================================================
-- Simulação SQL Server de uma plataforma de dados governada:
--   FONTES -> CATÁLOGO DE CONTROLE -> BRONZE -> SILVER -> GOLD -> PRODUTO SEMÂNTICO
--
-- As tabelas de controle representam catálogo, contratos, linhagem, execuções,
-- integração de fontes, classificação e políticas de acesso. As tabelas de dados
-- representam o fluxo de transformação. OneLake, Lakehouse, Warehouse, Data
-- Factory e Power BI são serviços de plataforma representados por metadados, não
-- criados pelo T-SQL.
-- TEORIA: ../../../certification/12-other-topics/04-fabric-architecture.md
-- REFERÊNCIA OFICIAL:
-- https://learn.microsoft.com/pt-br/fabric/fundamentals/data-lifecycle
-- =================================================================================
...
-- =================================================================================
-- PARTE 1: REGISTRO DA FONTE E PLANO DE CONTROLE DA PLATAFORMA
-- =================================================================================
...
VALUES
    (10, N'O-5001', 101, N'Contoso Brasil', '2026-08-09', N'Capacete Adventure', 2, 50.00, '2026-08-10T08:00:00', N'SQL-OLTP'),
    (10, N'O-5001', 101, N'Contoso Brasil', '2026-08-09', N'Capacete Adventure', 2, 50.00, '2026-08-10T08:05:00', N'SQL-OLTP'),
    (20, N'O-5002', 202, N'Northwind Bikes', '2026-08-09', N'Pneu Touring', 1, 80.00, '2026-08-10T08:01:00', N'SQL-OLTP'),
    (10, N'O-5003', 103, N'Fabrikam Retail', '2026-08-09', N'Garrafa Road', 0, 15.00, '2026-08-10T08:02:00', N'SQL-OLTP');
GO
...
VALUES
    (N'SQL-OLTP Orders', 'RELATIONAL', 'CDC_OR_WATERMARK', 'COPY', N'Lakehouse Bronze', N'Comércio'),
    (N'Partner inventory files', 'FILES', 'PIPELINE', 'SHORTCUT_OR_COPY', N'Lakehouse Bronze', N'Abastecimento');
...
VALUES
    (N'SQL-OLTP Orders', 'SOURCE', 'SOURCE', N'Comércio', 'CONFIDENTIAL', 'ACTIVE', 0),
    (N'Lakehouse Bronze Orders', 'LAKEHOUSE_TABLE', 'BRONZE', N'Comércio', 'CONFIDENTIAL', 'ACTIVE', 0),
    (N'Warehouse Silver Orders', 'WAREHOUSE_TABLE', 'SILVER', N'Comércio', 'CONFIDENTIAL', 'ACTIVE', 0),
    (N'Gold Sales Product', 'SEMANTIC_SOURCE', 'GOLD', N'Comércio', 'INTERNAL', 'CERTIFIED', 1),
    (N'Sales Semantic Model', 'SEMANTIC_MODEL', 'SERVING', N'Comércio', 'INTERNAL', 'CERTIFIED', 1);
...
VALUES
    ...
    (N'SQL-OLTP Orders', N'Pipeline Identity', 'READ', N'Janela incremental aprovada');

INSERT INTO lab_fabric_control.DataContracts
    (ContractName, Version, RequiredColumns, QualityRule, OwnerDomain)
VALUES
    (N'Sales Silver Contract', 'v1.0', N'TenantID,OrderID,CustomerID,OrderDate,Quantity,SalesAmount', N'Quantity > 0; SalesAmount >= 0; OrderID único por lote', N'Comércio'),
    (N'Sales Gold Contract', 'v1.0', N'TenantID,OrderDate,CustomerID,Quantity,SalesAmount', N'Somente linhas Silver certificadas são publicadas', N'Comércio');

INSERT INTO lab_fabric_control.LineageEdges (UpstreamItem, DownstreamItem, TransformationStep)
VALUES
    (N'SQL-OLTP Orders', N'Lakehouse Bronze Orders', N'Ingestão CDC/watermark preserva origem e metadados do lote'),
    (N'Lakehouse Bronze Orders', N'Warehouse Silver Orders', N'Deduplicar, validar contrato e colocar inválidos em quarentena'),
    (N'Warehouse Silver Orders', N'Gold Sales Product', N'Conformar clientes e publicar medidas prontas para negócio'),
    (N'Gold Sales Product', N'Sales Semantic Model', N'Expor medidas governadas e filtros por tenant');
GO

-- =================================================================================
-- PARTE 2: INGESTÃO NA BRONZE E REGISTRO DA EXECUÇÃO
-- =================================================================================
...
VALUES (N'pl_orders_to_bronze', N'SQL-OLTP Orders', N'batch-2026-08-10', '2026-08-10T09:00:00', 'RUNNING', 0);
...
-- =================================================================================
-- PARTE 3: VALIDAÇÃO DO CONTRATO SILVER E QUARENTENA
-- =================================================================================
...
;WITH Ranked AS (
    ...
    SELECT *, CASE WHEN rn > 1 THEN N'OrderID duplicado no lote'
                   WHEN Quantity <= 0 THEN N'Quantity deve ser maior que zero'
                   WHEN SalesAmount < 0 THEN N'SalesAmount deve ser zero ou maior' END AS RejectionReason
    FROM Ranked
)
INSERT INTO lab_fabric_silver.SalesQuarantine (BatchID, SourceRowID, OrderID, RejectionReason)
SELECT BatchID, SourceRowID, OrderID, RejectionReason FROM Decisions WHERE RejectionReason IS NOT NULL;
...
FROM Ranked WHERE rn = 1 AND Quantity > 0 AND SalesAmount >= 0;
GO

-- =================================================================================
-- PARTE 4: PRODUTO GOLD E SERVING SEMÂNTICO
-- =================================================================================
...
SELECT DISTINCT TenantID, CustomerID, CustomerName FROM lab_fabric_silver.SalesClean;
INSERT INTO lab_fabric_gold.FactSales (TenantID, OrderDate, CustomerKey, OrderID, Quantity, SalesAmount)
...
SELECT TenantID, OrderDate, SUM(Quantity), SUM(SalesAmount), 1 FROM lab_fabric_gold.FactSales
GROUP BY TenantID, OrderDate;
GO

SELECT TenantID, OrderDate, UnitsSold, SalesAmount
FROM lab_fabric_gold.SemanticSalesDaily WHERE TenantID = 10;
GO

-- =================================================================================
-- PARTE 5: LINHAGEM, ACESSO, CUSTO E GATES DE PROMOÇÃO
-- =================================================================================
UPDATE lab_fabric_control.PipelineRuns
SET RowsPublished = (SELECT COUNT(*) FROM lab_fabric_silver.SalesClean)
WHERE PipelineName = N'pl_orders_to_bronze' AND BatchID = N'batch-2026-08-10';

SELECT c.ItemName, c.ItemType, c.LayerName, c.OwnerDomain, c.Classification, c.LifecycleStatus, c.IsCertified
FROM lab_fabric_control.CatalogItems AS c ORDER BY c.LayerName, c.ItemName;
SELECT UpstreamItem, DownstreamItem, TransformationStep
FROM lab_fabric_control.LineageEdges ORDER BY UpstreamItem, DownstreamItem;
SELECT ItemName, PrincipalName, AccessLevel, RowFilter
FROM lab_fabric_control.AccessPolicies WHERE ItemName = N'Gold Sales Product';
GO

IF EXISTS (SELECT 1 FROM lab_fabric_silver.SalesClean WHERE Quantity <= 0 OR SalesAmount < 0)
    THROW 51040, 'Gate de qualidade do contrato Silver falhou.', 1;
IF NOT EXISTS (SELECT 1 FROM lab_fabric_control.LineageEdges WHERE UpstreamItem = N'Gold Sales Product' AND DownstreamItem = N'Sales Semantic Model')
    THROW 51041, 'Gate de linhagem do serving falhou.', 1;
IF NOT EXISTS (SELECT 1 FROM lab_fabric_control.CatalogItems WHERE ItemName = N'Sales Semantic Model' AND IsCertified = 1)
    THROW 51042, 'Gate de certificação do modelo semântico falhou.', 1;
IF EXISTS (SELECT 1 FROM lab_fabric_control.PipelineRuns WHERE Status <> 'SUCCEEDED')
    THROW 51043, 'Gate operacional do pipeline falhou.', 1;
SELECT N'PASSOU' AS FabricArchitectureGate,
       N'Catálogo, contrato, linhagem, acesso, pipeline e serving foram validados.' AS Result;
GO
